...
2. 在整份筆記的最末端，請 **務必** 使用以下 \`<atomic-notes>\` 的 XML（延標記語言）結構，為每一個標記 \`[[ ]]\` 的專有名詞產生獨立筆記內容。在內容中，必須給出明確的定義，且**務必增加對該概念的條列化（Bullet points）拆解**，分項詳細列出其核心機制、原則、應用步驟或具體場景，禁止使用大段落純文字描述，以求結構清晰、項目條列分明。
...
- **核心要素/運作邏輯 1**：條列化拆解說明 1...
- **應用場景/具體步驟 2**：條列化拆解說明 2...
- **關鍵原則/限制條件 3**：條列化拆解說明 3...
</note>
